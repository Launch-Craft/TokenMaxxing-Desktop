        ...
        <div className="flex flex-1 items-center gap-4">
          <div className="relative h-[180px] w-[180px] shrink-0">
            ...
            <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
              <span className="font-mono text-xl font-bold leading-none tabular">
                {formatCompact(total)}
              </span>
          ...
          <ul className="flex-1 space-y-2.5">
            {data.slice(0, 6).map((slice) => (
              <li key={slice.toolId} className="flex items-center gap-2.5 text-sm">
                ...
                />
                <span className="truncate text-muted-foreground">{slice.toolName}</span>
                <span className="ml-auto text-right">
                  <span className="block font-mono text-xs font-semibold tabular">
                    {slice.percentage}%
                  </span>
                  <span className="block font-mono text-[10px] tabular text-muted-foreground">
